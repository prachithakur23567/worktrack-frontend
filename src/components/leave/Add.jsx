...
const handleSubmit= async(e)=>{
    e.preventDefault();
    try{
        const response = await axios.post(`https://worktrack-backend.vercel.app/api/leave/add`,leave,{
            headers:{
 ...
 navigate(`/employee-dashboard/leaves/${user._id}`)
    ...
    <form onSubmit={handleSubmit} >
        ...
        <div>
            ...
            <textarea name="reason" placeholder='Reason' onChange={handleChange} className='mt-1 p-2 block w-full border border-gray-300 rounded-md' required></textarea>
        </div>
        </div>
        
        <button type='submit' className='w-full mt-6 bg-teal-600 hover:bg-teal-700 text-white font-bold py-2 px-4 rounded-md'>Add Leave </button>
    </form>
</div>
  )
}

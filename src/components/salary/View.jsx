...
const View = () => {
 ...
 const {user} = useAuth()
    const fetchSalaries = async()=>{
        ...
        try{
            const response = await axios.get(`https://worktrack-backend.vercel.app/api/salary/${id}/${user.role}`,{
                headers:{
...
fetchSalaries();
  ...
  {filteredSalaries.length >0 ?(
    <table className='w-full text-sm text-left text-gray-500'>
        <thead className='text-ws text-gray-700 uppercase bg-gray-50 border border-gray-200'>
            <tr>

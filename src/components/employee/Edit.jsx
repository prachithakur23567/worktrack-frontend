...
const Edit = () => {
    ...
    useEffect(()=>{
        const fetchEmployee = async()=>{
            ...
            try{
                const response = await axios.get(`https://worktrack-backend.vercel.app/api/employee/${id}`,{
                    headers:{
    ...
    const handleSubmit= async(e)=>{
        e.preventDefault()
        try{
        const response = await axios.put(`https://worktrack-backend.vercel.app/api/employee/${id}`,employee,{
            headers: {
                "Authorization":`Bearer ${localStorage.getItem('token')}`
            }
        })
    ...
    <div className='max-w-4xl mx-auto mt-10 bg-white p-8 rounded-md shadow-md'>
        ...
        <form onSubmit={handleSubmit}>
            ...
            <div>
                ...
                onChange={handleChange} placeholder='Designation' className='mt-1 p-2 block w-full border border-gray-300 rounded-md' required/>
            </div>
                        <div>
